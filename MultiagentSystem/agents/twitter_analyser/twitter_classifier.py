"""
Shared Twitter classification logic.
...
Used by twitter_collector.py before writing rows into SQLite.
"""
...
- "BULL": likely positive for BTC price.
- "BEAR": likely negative for BTC price.
- "NO_CORRELATION_TO_BTC": no meaningful BTC impact.

Confidence scale:
- "LOW": weak or indirect signal.
- "MIDDLE": noticeable but not decisive signal.
- "HIGH": strong/direct catalyst likely to affect BTC price.

Rules:
1) Classify every item.
2) Return exact enums only.
3) Preserve tweet_id exactly as provided.
"""
